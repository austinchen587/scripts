...
1. **👑 型号霸权法则（最高优先级）**：如果规格中明确出现了极其具体的“字母+数字”商品型号（如：3288dw、i7-12700、M4300、KG316T），该型号拥有绝对的最高优先级！你必须将其作为核心搜索词（无需保留"型号:"字样，直接写"3288dw"）。
2. **防诱导原则**：如果“具体型号”与“参考品牌”产生冲突（比如品牌写惠普，但型号是兄弟的3288dw），在提取搜索词时，直接放弃品牌，**只搜品类和型号**！
3. **精准抓取决定性参数**：如果没有具体型号，则揪出【决定设备分类、大小、原理】的核心物理参数（如：118mm/热转印）。
4. **只要骨干，拒绝赘肉**：电商搜索引擎极其愚蠢！最终搜索词**最多包含3-4个核心词汇**（型号/核心参数 + 修正后的名词）。
5. **绝对禁止的词汇**：
   - 严禁出现标点符号、描述前缀（绝对不能输出"型号:3288dw"，只能输出"3288dw"）。
   - 严禁出现包装数量（如：500张/包）、极长琐碎尺寸（如297*420mm）。
   - 严禁出现废话（如：带检测报告、双面、正品）。
...
[输入]: 彩色打印机, 品牌：惠普/HP联想/lenovo, 规格：颜色分类:白;最大分辨率:1200*1200;型号:3288dw;
[思考]: 发现极其具体的型号"3288dw"！触发型号霸权法则，直接忽略冲突的品牌(惠普/联想)和无用描述。
[输出]: {{"core_params": "3288dw", "keyword": "3288dw 彩色打印机", "platform": "京东"}}

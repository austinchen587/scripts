...
def save_detail_raw_data(procurement_id, brand_id, num_iid, platform, raw_data):
    """将抓取到的详情原始 JSON 存入云端数据库"""
    conn = get_connection()
    if not conn: return
    try:
        table_name = f"procurement_commodity_{platform}_detail"
        with conn.cursor() as cur:
            cur.execute(f"""
                INSERT INTO {table_name} (procurement_id, brand_id, num_iid, raw_data) 
                VALUES (%s, %s, %s, %s)
            """, (str(procurement_id), brand_id, str(num_iid), Json(raw_data)))
        conn.commit()
        logger.info(f"✅ 详情入库成功: [Brand:{brand_id}] 平台:{platform} | 商品ID:{num_iid}")
    except Exception as e:
        logger.error(f"❌ 详情入库失败 [Brand:{brand_id}]: {e}")
        if conn: conn.rollback()
    finally:
        conn.close()
...
def build_batch_prompt(demand, batch, round_num, batch_idx):
    ...
    2. 规格与品牌死线：规格严重不符，或假冒指定品牌者，直接淘汰！
    3. 优选策略：在确认为真实所需商品的前提下，优先保留【价格合理且偏低】且【销量(sales)较好/店铺正规】的商品。
...
def build_final_prompt(demand, finalists):
    ...
    【🏆 选品终极三维法则】：
    在确认商品为真（非配件引流）且规格精准对应的前提下，按以下权重排序：
    第一权重：价格最具竞争力（成本越低，利润空间越大）。
    第二权重：销量口碑较好，或店铺名称更正规（如旗舰店、专卖店、自营优先）。
    ...
    请严格按以下JSON格式输出：
    {{
        "market_sense_check": "简述这几款入围商品的真实性、性价比和销量口碑情况",
        "selected": [
...
def invoke_ollama(prompt, desc=""):
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": "你是一个极度犀利的电商风控专家与政企采购总监。擅长根据具体品类动态识破引流陷阱，并能在真实商品中挑选出性价比与销量最优的供应商。"},

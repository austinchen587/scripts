...
def clear_retry_placeholder(brand_id):
    """
    【终极防连环踩踏修复 - 增强唤醒版】
    1. 本地：UPDATE 状态为 ai_processing，并发送 NOTIFY 信号唤醒 AI 引擎。
    2. 云端：DELETE retry 占位符，防止同步倒灌。
    """
    if not brand_id: return
    local_conn = None
    cloud_conn = None
    try:
        # 1. 本地操作：锁定任务状态并“敲钟”唤醒 AI
        local_conn = psycopg2.connect(**DB_CONFIG)
        with local_conn.cursor() as cur:
            # 更新状态，让 AI 引擎的 data_loader 能够识别并读取该任务
            cur.execute("""
                UPDATE procurement_commodity_result 
                SET status = 'ai_processing', selection_reason = '爬虫已交单，等待AI决策...'
                WHERE brand_id = %s;
            """, (brand_id,))
            
            # 【核心修复】发出 PostgreSQL 异步通知信号
            # 信号频道必须为 'local_sku_channel'，与 result_05/local_listener.py 对应
            cur.execute("NOTIFY local_sku_channel, %s;", (f"brand_ready_{brand_id}",))
            
            local_conn.commit()
            logger.info(f"🔔 [通知] 本地状态已锁定，并已发送唤醒信号 (ID: {brand_id})")
            
        # 2. 云端操作：清理重试占位符
        from db_sync import CLOUD_DB_CONFIG
        cloud_conn = psycopg2.connect(**CLOUD_DB_CONFIG)
        with cloud_conn.cursor() as cloud_cur:
            cloud_cur.execute("DELETE FROM procurement_commodity_result WHERE brand_id = %s AND status = 'retry';", (brand_id,))
            cloud_conn.commit()
            
        logger.info(f"🧹 [闭环] 爬虫已交单！本地上锁 + 云端清理 (ID: {brand_id})，彻底阻断死循环！")
        
    except Exception as e:
        if local_conn: local_conn.rollback()
        if cloud_conn: cloud_conn.rollback()
        logger.error(f"清理占位符或发送唤醒信号失败: {e}")
    finally:
...
def update_sku_detail(procurement_id, sku, platform, specs_data):
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # 【情况 A：爬虫根本没找到对应的规格按钮】
            if specs_data.get("error") == "sku_mismatch":
                # 1. 标记 SKU 为废弃状态 (例如 fetch_status = -1)
                cur.execute("""
                    UPDATE procurement_commodity_sku 
                    SET fetch_status = -1, updated_at = CURRENT_TIMESTAMP
                    WHERE sku = %s AND platform = %s;
                """, (str(sku), platform))
                
                # 2. 修改 Result 表状态，直接告诉 AI 淘汰此商品
                cur.execute("""
                    UPDATE procurement_commodity_result 
                    SET status = 'ai_processing', 
                        selection_reason = '【爬虫物理淘汰】详情页内无法点中对应规格，价格失效，已自动剔除'
                    WHERE brand_id = (SELECT brand_id FROM procurement_commodity_sku WHERE sku = %s LIMIT 1);
                """, (str(sku),))
                logger.info(f"🚫 SKU {sku} 规格不匹配，已标记淘汰并交还 AI")
            ...
            # 【情况 B：正常抓取到了数据】
                ...
                cur.execute("""
                    ...
                    WHERE sku = %s AND platform = %s;
                """, (specs_json_str, str(sku), platform))
                
                cur.execute("""
                    UPDATE procurement_commodity_result 
                    SET status = 'ai_processing', selection_reason = '详情回采已完成，等待最终决策'
                    WHERE brand_id = (SELECT brand_id FROM procurement_commodity_sku WHERE sku = %s LIMIT 1);
                """, (str(sku),))
                logger.info(f"✅ [补签成功] SKU {sku} 详情已入库，任务状态已交还 AI")
            
            # 无论成功失败，都敲钟唤醒 AI 继续推进流程

...
def sync_table_download(table_name):
    """
    下载逻辑终极版：手动 Upsert (DELETE + INSERT)
    完美绕开 PostgreSQL 繁琐的的主键/唯一约束冲突
    """
    cloud_conn = None
    local_conn = None
    try:
        ...
        if pk_col and pk_col not in columns:
            logger.warning(f"⚠️ 表 {table_name} 中未找到指定的键 '{pk_col}'，将自动推断...")
            pk_col = None
        
        if not pk_col:
            if 'id' in columns:
                pk_col = 'id'
            elif 'procurement_id' in columns:
                pk_col = 'procurement_id'
            elif 'brand_id' in columns:
                pk_col = 'brand_id'
            else:
                pk_col = columns[0]
        ...
        with local_conn.cursor() as cur:
            ...
            local_conn.commit()
            logger.info(f"⬇️ [Sync] 表 {table_name} 完美同步完成 (处理 {len(rows)} 条)")

    except Exception as e:
        logger.error(f"❌ [Sync] 下载失败 {table_name}: {e}")

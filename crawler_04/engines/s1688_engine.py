...
class S1688Engine(BaseEngine):
    def search(self, keyword):
        ...
        # 确保在搜索页
        if 's.1688.com' not in self.tab.url:
        ...
        # === [新增] 核心修复：优先检测“无结果”标志 ===
        # 1688 无结果通常显示：“没有找到关于...的商品” 或 class="sm-no-result"
        ...
        if no_result_ele:
            ...
            return [] # 直接返回空，触发 main.py 的占位数据写入
        ...
        for page in range(1, max_page + 1):
            # --- 安全检查：防止误入详情页 ---
            if 'detail.1688.com' in self.tab.url or 'offer' in self.tab.url:

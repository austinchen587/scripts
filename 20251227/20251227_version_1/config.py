...
# ========== 第一阶段：关键词分类阈值 ==========
KEYWORD_THRESHOLDS = {
    "high_confidence": 0.85,      # 原值：>=0.85，太高了，直接结束
    "medium_confidence": 0.70,    # 原值：>=0.70（应该进入第二阶段）
    "low_confidence": 0.60        # 原值：>=0.60（应该进入第二阶段）
}

# ========== 第二阶段：余弦相似度验证阈值 ==========
COSINE_THRESHOLDS = {
    "high_confidence": 0.80,      # 高置信度阈值
    "acceptable": 0.65,           # 可接受阈值
    "requires_verification": 0.55 # 需要复核阈值
}

...
【严禁过度拆分(合并参数铁律)】：不管原表中的参数是跨越多行，还是使用了一长串带有编号的列表（例如写着"1、饰面... 2、海绵... 3、框架... 4、油漆..."），【绝对禁止】将它们拆分成多个独立的商品对象！你【必须】将它们合并，用分号(;)连接起来，统一填入该单一商品的“规格型号”或“备注”字段中！整个JSON输出中，该商品只能作为一个单独的对象出现1次！绝对不允许自行给商品名称添加“_1”、“_2”等后缀！
